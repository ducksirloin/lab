...
f = open(fname5,"w")
for i in range(361):
	f.write("~/tnagai/programs/gromacs-4.6.1-d-mpi-deb/bin/mdrun_mpi_d  -s  charmm" + str(i) + ".tpr  > logmd" + str(i) + ".log\n")
f.close()

fname6 = "do_ener_sh.csh"
f = open(fname6,"w")
for i in range(361):
	f.write("~/tnagai/programs/gromacs-4.6.1-d/bin/g_energy_d -f  ener.edr  -s charmm" + str(i) + ".tpr -o ener.out -xvg none > log.ener <<EOF\n")
