...
for f in open(fname,"r"):
    ...
    for i in range(len(x)):
        if x[i] == "Etot":
            ...
            f11.write(str(ang[10]) + " " + str(x[i+2]) + "\n")
            ang[10] += 1
...
for f in open(fname10,"r"):
    x = f.split()
    EGB.append(x[1])
for f in open(fname11,"r"):
    x = f.split()
    ERISM.append(x[1])

a = -180
fname12 = "result_of_3DRISM.txt"
f = open(fname12,"w")
f.write("dih  Etot    EPtot   BOND   ANGLE  DIHED   1-4NB  1-4EEL  VDWAALS  EELEC  ERISM\n")
for i in range(0,361):
    f.write(str(a) + " " + str(Etot[i]) + " " + str(EPtot[i]) + " " + str(BOND[i]) + " " + str(ANGLE[i]) + " " + str(DIHED[i]) + " " + str(NB[i]) + " " + str(EEL[i]) + " " + str(VDWAALS[i]) + " " + str(EELEC[i]) + " " + str(ERISM[i]) + "\n")
    a += 1
f.close()
